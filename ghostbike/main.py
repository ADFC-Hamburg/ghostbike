...
async def lifespan(app: FastAPI):
    # Startup
    json_file_path = os.getenv("GHOSTBIKE_JSON_PATH", "Ghostbike-Hamburg-Tabelle.json")
    if os.path.exists(json_file_path):
        await initialize_all_data(json_file_path)
    yield
...
#
#    DB = SQLiteEngine(path=os.getenv("GHOSTBIKE_SQLITE_PATH", "ghostbike.sqlite3"))
# else:
...
tables = [
    ...
    TableConfig(
        ...
        visible_columns=[User.username, User.email, User.active, User.superuser],
        visible_filters=[User.username, User.email, User.active, User.superuser],
    ),
    TableConfig(
        table_class=Guardian,
        visible_columns=[Guardian.name, Guardian.email, Guardian.phone],
        visible_filters=[Guardian.name, Guardian.email, Guardian.phone],
    ),
    TableConfig(
        table_class=StreetType,
        menu_group="Lookup-Tabellen",
        link_column=StreetType.key,
        visible_columns=[StreetType.key, StreetType.name],
        visible_filters=[StreetType.key, StreetType.name],
    ),
    TableConfig(
    ...
    TableConfig(
        table_class=GuardianGhostbike,
        visible_columns=[GuardianGhostbike.ghostbike, GuardianGhostbike.guardian],
        visible_filters=[GuardianGhostbike.ghostbike, GuardianGhostbike.guardian],
    ),
    TableConfig(
